...
def required(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"VALIDATION FAILED: {name} is missing")
    return value
...
def check_image(url: str) -> None:
    ...
    try:
        ...
        # Some CDNs reject HEAD. Fall back to a small GET without storing the body.
...
def main() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            "Medium credential check failed. Existing Medium integration tokens may still work, "
            "but Medium no longer issues new integration tokens/new integrations. "
            f"Underlying result: {exc}"
        ) from exc

    if not medium_user_id:
        raise RuntimeError("VALIDATION FAILED: MEDIUM_USER_ID is missing")
