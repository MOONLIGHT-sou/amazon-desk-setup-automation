...
def load_products(path=PRODUCTS_FILE):
    ...
    for product in products:
        ...
        if product["used"].lower() not in {"yes", "no"}:
            raise RuntimeError(f"Invalid used value for {product['product_id']}: {product['used']!r}")
        if not re.fullmatch(r"https?://[^\s]+", product["amazon_link"]):
            raise RuntimeError(f"Invalid Amazon link for {product['product_id']}: {product['amazon_link']!r}")
    return products
...
def load_product_facts(product_id):
    try:
        ...
        raise RuntimeError(f"Content safety failed: verified product facts are unavailable for {product_id}: {exc}") from exc
    ...
    if not isinstance(safe_facts, list) or not all(isinstance(item, str) and item.strip() for item in safe_facts):
        raise RuntimeError(f"Content safety failed: safe_facts are missing or invalid for {product_id}.")
    return facts
...
def select_for_run(products, test_mode):
    ...
    print("Selection intelligence: selected " f"{selected['product_id']} — {selected['product_name']} " f"(decision={result['decision']}, score={result['opportunity_score']}, " f"coverage={result['evidence_coverage']}, confidence={result['confidence']})")
...
def load_review_authorization(product_id):
    ...
    if review.get("status") != "APPROVED":
        raise RuntimeError("SAFETY STOP: human review has not approved this product. " f"status={review.get('status')!r}")
...
def infer_profile(product):
    ...
    for profile_name, profile in PRODUCT_PROFILES.items():
        if profile_name == "keyboard" and {"keyboard", "storage"}.issubset(tokens):
            continue
        if all(keyword in tokens for keyword in profile["keywords"]):
            return profile_name, profile

    raise RuntimeError(f"Content quality failed: no approved product profile exists for {product['product_id']} ({product['product_name']}). Add a reviewed profile before generating production content.")
...
def build_content(product):
    ...
    medium_subtitle = f"A practical {category.lower()} decision guide focused on fit, tradeoffs, and everyday use."
    ...
    main_pin_description = f"Planning a cleaner {category.lower()}? Use this {name} buying checklist to compare fit, space, compatibility, and everyday usefulness before adding another accessory. {LINK_DISCLOSURE} Check the current listing before buying. {REQUIRED_DISCLOSURE}"
    product_pin_description = f"{name} is a focused {category.lower()} option to evaluate when it solves a real desk problem. Check the seller's current specifications, dimensions, compatibility, price, and reviews before deciding. {LINK_DISCLOSURE} {REQUIRED_DISCLOSURE}"
...
def verify_link_disclosure(section, link):
    ...
    if LINK_DISCLOSURE.lower() not in nearby.lower():
        raise RuntimeError("Content compliance failed: affiliate link does not have a nearby paid-link disclosure.")


def verify_content(content, product):
    if not content.strip():
        raise RuntimeError("Generated content is empty.")
    required_content = (product["product_id"], product["product_name"], product["category"], product["amazon_link"], "MEDIUM", "PINTEREST MAIN PIN", "PINTEREST PRODUCT PIN", "CONTENT QUALITY", REQUIRED_DISCLOSURE, LINK_DISCLOSURE, "Editorial status: This package is generated from structured product data and requires human review before publication.")
    for required_text in required_content:
        if not required_text or required_text not in content:
            raise RuntimeError(f"Content verification failed: missing '{required_text}'.")
    facts = load_product_facts(product["product_id"])
    variant = facts.get("variant", "").strip()
    if variant and variant not in content:
        raise RuntimeError(f"Content verification failed: verified product variant '{variant}' is missing.")
    for safe_fact in facts["safe_facts"]:
        if safe_fact not in content:
            raise RuntimeError(f"Content verification failed: verified safe fact '{safe_fact}' is missing.")
    assert_no_pattern(content, (PLACEHOLDER_PATTERN,), "Content verification failed: unresolved placeholder detected.")
    assert_no_pattern(content, BANNED_CLAIMS, "Content safety failed: unsupported or high-risk claim detected.")
    assert_no_pattern(content, FABRICATED_EXPERIENCE, "Content integrity failed: fabricated personal experience detected.")
    ...
    if medium_words < MIN_MEDIUM_WORDS:
        raise RuntimeError(f"Content quality failed: MEDIUM section has {medium_words} words; minimum is {MIN_MEDIUM_WORDS}.")
    ...
    profile_name, profile = infer_profile(product)
    for section_name, section in (("MEDIUM", medium), ("PINTEREST MAIN PIN", main_pin), ("PINTEREST PRODUCT PIN", product_pin)):
        if section.count(product["product_name"]) < 1:
            raise RuntimeError(f"Content quality failed: {section_name} does not mention the product.")
    for phrase in profile["sections"][:3]:
        if phrase.lower() not in medium.lower():
            raise RuntimeError(f"Content specificity failed: MEDIUM is missing approved product-specific consideration '{phrase}'.")
    ...
    if disclosure_count < 4:
        raise RuntimeError("Content compliance failed: required Amazon Associate statement is not repeated in all package surfaces.")
    ...
    print(f"Content quality: PASS (Medium={medium_words} words, Main Pin={main_pin_words} words, Product Pin={product_pin_words}, profile={profile_name}, disclosures={disclosure_count})")
...
def generate_and_verify(product, output_root, write_output=True):
    ...
    if os.path.exists(content_file):
        raise RuntimeError(f"SAFETY STOP: production content already exists for {product['product_id']}; refusing to overwrite it.")
...
def save_products_atomic(products, path=PRODUCTS_FILE):
    ...
    file_descriptor, temporary_path = tempfile.mkstemp(prefix=".products-", suffix=".tmp", dir=directory)
    try:
...
def mark_product_used(products, product_id):
    ...
    if target["used"].lower() != "no":
        raise RuntimeError(f"SAFETY STOP: target {product_id} is no longer unused ({target['used']!r}).")
    target["used"] = "Yes"
...
def main():
    ...
    if not test_mode and product_id != target_product_id:
        raise RuntimeError("SAFETY STOP: intelligent selector target does not match the explicitly authorized production target. " f"selected={product_id}, authorized={target_product_id}")
    if not test_mode:
        load_review_authorization(product_id)
        expected_asin = load_verified_asin(product_id)
        try:
            validate_affiliate_link(selected_product["amazon_link"], expected_asin, AMAZON_ASSOCIATE_TAG)
        except ValueError as exc:
            raise RuntimeError(f"SAFETY STOP: affiliate link validation failed for {product_id}: {exc}") from exc
    ...
    content_file = generate_and_verify(selected_product, output_root, write_output=write_output)
    if content_file:
    ...
    current_product = next((product for product in current_products if product["product_id"] == product_id), None)
    if current_product is None:
        raise RuntimeError(f"SAFETY STOP: target {product_id} disappeared before mutation.")
    if current_product["used"].lower() != "no":
        raise RuntimeError(f"SAFETY STOP: target {product_id} changed before mutation ({current_product['used']!r}).")
